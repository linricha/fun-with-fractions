...
public class BFCalcSystem {
  ...
  /**
   * int number representing division.
   */
  public static final int DIVIDE = 1;

  /**
   * int number representing multiplication.
   */
  public static final int MULTIPLY = 2;

  /**
   * int number representing addition.
   */
  public static final int ADD = 3;

  /**
   * int number representing subtraction.
   */
  ...
  /**
   * Returns the corresponding BigFraction based on str and record. Prints an error if a
   *  lowercase letter in str is not initialized in record and returns null.
   *
   * @param str A string representing a fraction,
   *  i.e. a string that passes as true/not 0 when put into the method checkFrac in BFCalcInput.
  ...
  public static BigFraction giveBigFrac(String str, PrintWriter error, BFRegisterSet record) {
    ...
    if (wholeNum) {
      ...
      // Checks if an lowercase letter register is used in the numerator
      // and is the only thing in the numerator.
      if (BFCalcInput.checkLowLetters(str.charAt(0))) {

        // Checks if that register has a valid value stored in record / if it is initiated.
        if (record.checkRecord(str.charAt(0))) {
          // need to make check that record is filled for that area.
          return record.get(str.charAt(0));
        } else { // prints error if not.
          error.println("*** ERROR [Invalid Expression: Register value not initiated] ***");
          error.flush();
          return null;
        } // if/else
      } else { // Return a whole number.
   ...
   *
   * @param line A tokenized array of Strings that passes as true when put into the
   *  method orderMatters stored in BFCalcInput.
  ...
  public static int[] operatorAssign(String[] line) {
    ...
    for (int j = 0; j < orderOfOperations.length - 1; j++) {
      ...
      switch (line[(j * 2) + 1].charAt(0)) {
        case '*':
          orderOfOperations[j] = DIVIDE;
          doFirstCount++;
          break;
        case '/':
          orderOfOperations[j] = MULTIPLY;
          doFirstCount++;
          break;
        case '+':
          orderOfOperations[j] = ADD;
          break;
        case '-':
          orderOfOperations[j] = SUBTRACT;
          break;
        default:
          break;
      } // switch
  ...
  /**
   * Returns an array BigFractions corresponding to line and record. Returns null if one
   * of the BigFractions is null as decided by giveBigFrac(String[], PrintWriter,
   * BFRegisterSet).
   *
   * @param line
   *  A tokenized array of Strings that passes as true when put into the method
   *  orderMatters stored in BFCalcInput.
   * @param error A PrintWriter that prints errors.
   * @param record A register containing the values that some lowercase letter keys represent.
   * @return An array of BigFractions.
   */
  public static BigFraction[] changeToBigFractions(
      String[] line, PrintWriter error, BFRegisterSet record) {
    int lineLen;
    ...
    for (int i = 0; i < bigFracsOnly.length; i++) {
      ...
      if (!Objects.equals(giveBigFrac(line[2 * i], error, record), null)) {
  ...
  /**
   * Simplifies simplifiedExpression by doing multiplication and division on the
   *  BigFractions first, adhereing to the order of operations when it involves
   *  multiplication, divison, addition, and subtraction.
   *
   * @param simplifiedExpression
   *  An array of BigFraction used to derive a single BigFraction by simplifying
   *  the BigFractions using the operators in int[] operators.
   * @param operators An array that represents of ints that represent 4 different
   *  kind of operators, derived from the operatorAssign method.
   * @param calcButton A BFCalculator used to do math
   */
  public static void multiplyFracsFirst(
      BigFraction[] simplifiedExpression, int[] operators, BFCalculator calcButton) {
    for (int j = 0, k = 0; 0 < operators[operators.length - 1];) {
      ...
      if (operators[j] <= 2) {
        ...
        j++;
      } else { // moves to the next BigFraction and operator if
               // the current operator is not multiplication or division
        j++;
        k++;
      } // if/else
    } // for
  ...
  /**
   * Adds and subtract all the BigFractions to get the final value of the all the BigFractions
   * together based on operators. Final value will be stored as lastValue in calcButton.
   *
   * @param simplifiedExpression
   *  An array of BigFraction used to derive a single BigFraction by simplifying
   *  the BigFractions using the operators in int[] operators. Multiplication and Division
   *  from multiplyFracsFirst should have been already done to this.
   * @param operators
   *  An array that represents of ints that represent 4 different kind
   *  of operators, derived from the operatorAssign method.
   * @param calcButton A BFCalculator used to do math.
   */
  public static void addAllFracLeft(
      BigFraction[] simplifiedExpression, int[] operators, BFCalculator calcButton) {
   ...
   */
  public static void calcTypeMessaging(
      boolean quickCalc, PrintWriter pen, String[] line, String message) {
  ...
  /**
   * Reads from the input and prints errors if and error has occured as designed
   * by orderMatters of BFCalcInput. Otherwise, prints a message based on quickcalc
   * or prints the result of an expression as read by input.
   *
   * @param quickCalc A boolean saying if QuickCalculation will take place or not.
   ...
   * @param cabinet A BFRegisterSet that stores the corresponding lowercase
   *  letter keys and their values.
   * @param input A Scanner that reads the input.
   * @param result
   *  A boolean array of size one that stores the result of whether or not
   *  the method returned at the end.
   */
  public static void runProgram(
      boolean quickCalc, PrintWriter pen, PrintWriter error, BFCalculator calcButton,
      BFRegisterSet cabinet, Scanner input, boolean[] result) {

    String[] stored = readIt2(input);

    // Checks if the inputs are of correct syntax.
    if (!BFCalcInput.orderMatters(stored, error, quickCalc)) {
      result[0] = false;
      return;
    } // if

    // Checks if the QUIT command has been typed as an input.
    if (BFCalcInput.checkQUIT(stored[0])) {
      result[0] = false;
      return;
    } // if

    // Checks if the STORE command has been typed as an input.
    if (BFCalcInput.checkSTORE(stored[0])) {
    ...
    if (BFCalcInput.checkFrac(stored[0], error, stored, quickCalc) > 0) {
      ...
      if (Objects.equals(simplifiedExpression, null)) {
        result[0] = false;
        return;
      } // if
   ...
   * @param cabinet
   *  A BFRegisterSet that stores the corresponding lowercase letter keys and their values.
   * @param args Command-Line Arguments.
   */
  public static void quickCalculatorStart(
      PrintWriter pen, PrintWriter error, Scanner look, BFCalculator calcButton,
      BFRegisterSet cabinet, String[] args) {
    boolean[] result = {true};
  ...
  } // QuickCalculatorStart
    //    (PrintWriter, PrintWriter, Scanner, BFCalculator, BFRegisterSet, String[])
   ...
   *  A BFRegisterSet that stores the corresponding lowercase letter keys and their values.
   */
  public static void interactiveCalculatorStart(
      PrintWriter pen, PrintWriter error, Scanner look, Scanner open,
      BFCalculator calcButton, BFRegisterSet cabinet) {
    boolean[] result = {true};
    ...
    closeObjects(pen, error, look);
    open.close();
  } // InteractiveCalculatorStart
    //    (PrintWriter, PrintWriter, Scanner, Scanner, BFCalculator, BFRegister)
} // class BFCalcSystem
